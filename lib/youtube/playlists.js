 ...
 */
/**
 * Sort videos newest-first by uploadDate (YYYYMMDD string from yt-dlp).
 * Videos with no uploadDate land at the end. Tie-breaker: original order
 * (Array.prototype.sort is stable in Node 12+).
 *
 * Returns a NEW array; doesn't mutate the input.
 */
function sortByUploadDateDesc(videos) {
  return videos.slice().sort((a, b) => {
    const ad = a.uploadDate || '';
    const bd = b.uploadDate || '';
    if (!ad && !bd) return 0;
    if (!ad) return 1;   // a (no date) → end
    if (!bd) return -1;  // b (no date) → end
    return bd.localeCompare(ad);  // descending string compare (YYYYMMDD)
  });
}
...
class Playlists {
  ...
  load(file) {
    ...
    try {
      ...
      // Defensive: re-sort persisted videos on load. Older playlist files
      // (pre-sort-feature) are in YouTube playlist order (oldest first);
      // this brings them in sync with the new contract without forcing
      // the user to re-crawl. Newer files re-saved by updateVideos are
      // already sorted so this is a no-op.
      for (const p of this.playlists) {
        if (Array.isArray(p.videos) && p.videos.length > 1) {
          p.videos = sortByUploadDateDesc(p.videos);
        }
      }
      return true;
    } catch (err) {
   ...
   *
   * Videos are persisted sorted by uploadDate descending (newest first).
   * Items without an uploadDate land at the end (treated as "unknown,
   * probably old or broken"). Sorting at write-time means every consumer
   * (UI, MCP tools, future API callers) sees the same order without
   * having to re-sort.
   */
  updateVideos(id, newVideos) {
    ...
    p.videos = sortByUploadDateDesc(merged);
...
module.exports = {
  ...
  extractPlaylistId, sortByUploadDateDesc,
  SCHEMA_VERSION, DEFAULT_CLEANUP_DAYS,
};
