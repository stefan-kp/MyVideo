...
class Queue {
  ...
  add(input) {
    ...
    if (input.source !== 'local' && input.source !== 'mediathek') {
      throw new Error('queue.add: source must be "local" or "mediathek"');
    }
    ...
    // Deduplication: same (source, contentId|url) already in queue → reject.
    // Identity is the underlying media reference, not the per-add UUID.
