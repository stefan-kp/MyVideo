...
/**
 * Resolve the FRITZ!Box's native RTSP URL for a tuner without starting
 * any local transcode. Used by the diag UI's 'In VLC öffnen' link so
 * VLC streams directly from the FRITZ!Box, bypassing the Pi entirely.
 *
 * Returns null if FRITZ!Box is not configured.
 */
async function resolveDirectRtspUrl(tunerId) {
  const session = getSession();
  if (!session) return null;
  _resolver = _resolver || new M3uResolver({ session });
  return _resolver.getRtspUrl(tunerId);
}
