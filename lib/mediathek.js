...
// Nachrichten von heute fuer die Summary (alle Sender, nicht region-spezifisch)
const SUMMARY_TOPICS = [
  { query: 'ZIB 1', byTopic: true },
  { query: 'ZIB 2', byTopic: true },
  { query: 'ZIB Flash', byTopic: true },
  { query: 'ZIB 9:00', byTopic: true },
  { query: 'ZIB 11:00', byTopic: true },
  { query: 'ZIB 13:00', byTopic: true },
  { query: 'ZIB 17:00', byTopic: true },
  { query: 'Nachrichten', byTopic: true },
...
async function searchTodaysNews() {
  ...
  const searchResults = await Promise.all(
    SUMMARY_TOPICS.map(({ query, byTopic }) =>
      (byTopic ? searchByTopic(query) : search(query)).catch(() => [])
    )
  );
  ...
  for (const batch of searchResults) {
    for (const r of batch) {
      if (r.timestamp >= todayStartTs && !seen.has(r.url)) {
        seen.add(r.url);
        results.push(r);
      }
    }
