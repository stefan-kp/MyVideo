...
async function fetchSubtitleText(url) {
  debug(`Lade Untertitel: ${url}`);
  const response = await axios.get(url, { timeout: 10000 });
  const xml = response.data;
  debug(`Untertitel geladen: ${xml.length} Zeichen, Content-Type: ${response.headers['content-type']}`);
  debugTruncated('Untertitel XML Anfang', xml, 500);
  ...
  debug(`Untertitel extrahiert: ${paragraphs.length} Paragraphen, ${result.length} Zeichen`);
  return result;
}
