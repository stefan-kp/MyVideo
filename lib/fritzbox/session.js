...
class FritzboxSession {
  ...
  async _login() {
    ...
    let challengeResp;
    try {
      challengeResp = await this.http.get(challengeUrl, { timeout: 5000 });
    } catch (err) {
      throw new Error(`FRITZ!Box unreachable at ${this.host}: ${err.message}`);
    }
    ...
    const loginUrl = `http://${this.host}/login_sid.lua?version=2&username=${encodeURIComponent(this.user)}&response=${encodeURIComponent(response)}`;
    let loginResp;
    try {
      loginResp = await this.http.get(loginUrl, { timeout: 5000 });
    } catch (err) {
      throw new Error(`FRITZ!Box login request failed: ${err.message}`);
    }
