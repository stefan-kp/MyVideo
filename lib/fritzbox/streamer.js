...
function copyArgs(rtspUrl, outDir) {
  return [
    ...
    '-rtsp_transport', 'udp',
    '-buffer_size', '8388608',
    // 10s analyse + 50 MB probe so FFmpeg sees a full SPS/PPS cycle and can
    // disambiguate H.264 from PSI/teletext PIDs before stream selection.
    '-analyzeduration', '10000000',
    '-probesize', '50000000',
    '-i', rtspUrl,
    // Map first H.264 video + first audio explicitly, ignore subtitle/teletext/etc.
    '-map', '0:v:0', '-map', '0:a:0?',
    '-ignore_unknown',
    '-c:v', 'copy',
...
function transcodeArgs(rtspUrl, outDir) {
  return [
    '-loglevel', 'warning',
    // Input robustness: discard corrupt packets, regenerate PTS, ignore decoder
    // errors. Don't use +nobuffer here - we want FFmpeg to smooth UDP jitter.
    '-fflags', '+discardcorrupt+genpts',
    '-err_detect', 'ignore_err',
    '-rtsp_transport', 'udp',
    '-buffer_size', '8388608',
